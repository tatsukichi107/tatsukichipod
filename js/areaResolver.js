...
(function () {
  ...
  if (!AM) {
    ...
    window.TSP_AREA = { resolveAreaId: () => "NEUTRAL" };
    return;
  }
  ...
  function isSeaAreaId(areaId) {
    return !!areaId && (areaId.indexOf("SS_") === 0 || areaId.indexOf("SN_") === 0);
  }
  ...
  function resolveAreaId(temp, hum, lightOrDepth) {
    ...
    if (h === 100) {
      ...
      const m = SEA_MAP[side];
      return m[l] || (side === "south" ? "SS_MID" : "SN_MID");
    }
    ...
    // ガード：定義漏れはNEUTRAL
    if (!AREAS[areaId]) {
      console.warn("[resolveAreaId] unknown id:", areaId, { temp: t, hum: h });
      return "NEUTRAL";
    }
