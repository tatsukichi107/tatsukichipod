/* FILE: css/app.css */
/* =========================================================
   TalisPod v0.79 - app.css（日本語/English 併記対応）
   方針：
   - 既存の見た目は維持
   - 併記用の小さなヘルパー（.tsp-inline / .tsp-dual / .btn-dual）を追加
   - タブは英語のみ（絵文字アイコンは表示）
   - 最悪環境の「暗い絵文字パラパラ」も維持（.tsp-darkfall）
   ========================================================= */
...
@keyframes tspDarkFall{
  0%{
    transform: translate3d(0,-10px,0) rotate(0deg) scale(var(--tspS,1));
    opacity:0;
  }
  10%{ opacity:0.9; }
  100%{
    transform: translate3d(var(--tspDX,0px), var(--tspDY,220px), 0) rotate(12deg) scale(var(--tspS,1));
    opacity:0;
  }
}
